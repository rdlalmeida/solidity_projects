...
// Run the following tests only if the current network's name is in the set of defined development networks, i.e.,
// local emulated blockchains as opposed to remote test networks (NOTE: The 3rd branch of the tertiary operator is at the
// bottom of this file)
...
developmentChains.includes(network.name)
    ? describe("FundMe", async function () {
        ...
        */
              ...
              mockV3AggregatorInstance = await deployments.get(
                  "MockV3Aggregator"
              )
          ...
          describe("fund", async function () {
              ...
              it("Adds founder to array of founders", async function () {
                  ...
                  const founder = await fundMeContract.getFounder(0)
          ...
          describe("withdraw", async function () {
              ...
              it("withdraw ETH from a single founder", async function () {
                  ...
                  const startingFundMeBalance =
                      await networkProvider.getBalance(fundMeInstance.address)
                  const startingDeployerBalance =
                      await networkProvider.getBalance(deployer)
                  ...
                  const gasPrice = Number(
                      transactionReceipt.gasPrice.toString()
                  )
              ...
              it("CheaperWithdraw testing for a single founder", async function () {
                  // Arrange
                  const startingFundMeBalance =
                      await networkProvider.getBalance(fundMeInstance.address)
                  const startingDeployerBalance =
              ...
              it("allows us to withdraw from multiple founders", async function () {
                  ...
                  for (let i = 1; i < numSigners; i++) {
                      ...
                      const fundMeConnectedContract =
                          await fundMeContract.connect(signerAccounts[i])
                  ...
                  await expect(fundMeContract.getFounder(0)).to.be.reverted
              ...
              it("CheaperWithdraw testing for multiple founders", async function () {
                  const signerAccounts = await ethers.getSigners()
              ...
              it("Only allows the owner to withdraw", async function () {
                  ...
                  const attackerConnectedContract =
                      await fundMeContract.connect(attacker)
          ...
          describe("fallback", async function () {
              it("should invoke the fallback function when a non-existent function is attempted", async function () {
                  ...
                  const fundedAddress = await fundMeContract.getFounder(0)
          ...
          describe("Getters test", async function () {
              this.beforeEach(async function () {
                  // Use the deployer account to fund the contract
                  const deployerSigner = (await ethers.getSigners())[0]
                  const connectedContract = await fundMeContract.connect(
                      deployerSigner
                  )
                  const transactionResponse = await connectedContract.fund({
                      value: sendValue,
                  })
                  await transactionResponse.wait(1)
              })

              it("Should get the correct owner address", async function () {
                  const owner = await fundMeContract.getOwner()

                  assert.equal(owner, deployer)
              })

              it("Should get the single founder for the contract and it should match the deployer account", async function () {
                  const founder = await fundMeContract.getFounder(0)

                  assert.equal(founder, deployer)
              })

              it("Should retrieve the sendValue as amount funded by the deployer account", async function () {
                  const founder = await fundMeContract.getFounder(0)

                  const amountFunded =
                      await fundMeContract.getAddressToAmountFunded(founder)
